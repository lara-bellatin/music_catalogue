...
async def get_person_by_id(id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get person by ID: {str(e)}")
...
async def search_person(query: str = Query(min_length=2, max_length=50)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to search for person: {str(e)}")
    except ValidationError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def bulk_create_persons(people_data: List[PersonCreate]):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create people in bulk: {str(e)}")
    except ValidationError as e:
        raise HTTPException(status_code=400, detail=str(e))
