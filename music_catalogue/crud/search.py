...
async def unified_search(
    query: str, entity_types: Optional[List[EntityType]] = None, limit: int = 20, offset: int = 0
    ...
    Args:
        ...
        offset (int, optional): The number of results to skip for pagination

    Returns:
        PaginatedSearchResponse: Paginated search results with total count
    ...
    try:
        ...
        if entity_types:
            search_query = search_query.in_("entity_type", [et.value for et in entity_types])

        res = await search_query.execute()

        total_count = res.data[0]["total_count"] if res.data else 0
        results = _parse_list(UnifiedSearchResult, res.data)
