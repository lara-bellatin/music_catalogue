...
async def get_person_by_id(id: str):
    ...
    """
    try:
        person = await Person.get_by_id(id)
        if not person:
...
async def search_person(query: str = Query(min_length=2, max_length=50)):
    ...
    """
    try:
        return await Person.search(query)
    except APIError as e:
        raise HTTPException(
...
async def create_person(person_data: PersonCreate):
    ...
    """
    try:
        return await Person.create(person_data)
    except APIError as e:
        raise HTTPException(
...
async def bulk_create_persons(people_data: List[PersonCreate]):
    ...
    """
    try:
        return [await Person.create(person_data) for person_data in people_data]
    except APIError as e:
        raise HTTPException(
