-- Add offset-based pagination to unified_search
drop function if exists unified_search;

create function unified_search(query_text text, fetch_limit int default 20, fetch_offset int default 0)
returns table (
...
as $$
    ...
    work_matches as (
        ...
        order by rank desc
        limit fetch_limit + fetch_offset
    ),
    version_matches as (
    ...
    counted as (
        select entity_type, entity_id, display_text, secondary_text, rank,
